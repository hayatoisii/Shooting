...
void Player::Initialize(KamataEngine::Model* model, KamataEngine::Camera* camera, const KamataEngine::Vector3& pos) {
    ...
    // hp: increase to allow 4 hits
    hp_ = 4;
    isDead_ = false;
    shotTimer_ = 0;

    // store starting HP for resets
    // (we can use ResetHP to set hp_ back to this value)

    // init hit shake
    hitShakeTimer_ = 0.0f;
    hitShakeDuration_ = 0.0f;
    hitShakeMagnitude_ = 0.0f;
    isHitShaking_ = false;
    savedTranslation_ = worldtransfrom_.translation_;
    savedRotation_ = worldtransfrom_.rotation_;
}

void Player::ResetHP() {
    hp_ = 4; // reset to initial value
    isDead_ = false;
}
...
void Player::OnCollision() {
    // isDead_ = true; // 即死
    hp_--;
    // start a heavy ship shake
    StartHitShake(40.0f, 1.2f); // 40 frames of heavy shaking
    if (hp_ <= 0) {
        isDead_ = true;
    }
}

void Player::StartHitShake(float durationFrames, float magnitude) {
    // Save current transform as center if not already
    if (!isHitShaking_) {
        savedTranslation_ = worldtransfrom_.translation_;
        savedRotation_ = worldtransfrom_.rotation_;
        isHitShaking_ = true;
    ...
    hitShakeDuration_ = durationFrames;
    hitShakeTimer_ = durationFrames;
    hitShakeMagnitude_ = magnitude;
}
...
void Player::Update() {
    ...
    // Apply hit shake if active: positional only (no rotation)
    if (hitShakeTimer_ > 0.0f) {
        // Before applying shake, reset to saved center transform
        worldtransfrom_.translation_ = savedTranslation_;
        worldtransfrom_.rotation_ = savedRotation_; // keep rotation stable

        auto randf = []() { return (static_cast<float>(MT::GetRand()) / RAND_MAX * 2.0f - 1.0f); };

        // compute decayed magnitude (linear decay)
        float curMag = hitShakeMagnitude_;
        if (hitShakeDuration_ > 0.001f) {
            curMag = hitShakeMagnitude_ * (hitShakeTimer_ / hitShakeDuration_);
        }

        // positional shake only: left/right and up/down using decayed magnitude
        float shakeX = randf() * curMag;
        float shakeY = randf() * curMag;

        worldtransfrom_.translation_.x += shakeX;
        worldtransfrom_.translation_.y += shakeY;

        hitShakeTimer_ -= 1.0f;
    } else {
        if (isHitShaking_) {
            // Shake ended, restore to saved transform exactly
            worldtransfrom_.translation_ = savedTranslation_;
            worldtransfrom_.rotation_ = savedRotation_;
            isHitShaking_ = false;
        }
        hitShakeTimer_ = 0.0f;
    }
